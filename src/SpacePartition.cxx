...
namespace Engine {
    ...
    void SpacePartition::checkOverlapSize() {
        ...
        if (_overlap * 2 > subfieldSizeX || _overlap * 2 > subfieldSizeY) {
            ...
            oss << "SpacePartition::checkOverlapSize- subfield sizes: " << subfieldSizeX << "/" << subfieldSizeY
                << " from global: " << _world->getConfig().getSize() << " and owned area: " << _ownedArea
                << " must be at least twice the value of overlap: " << _overlap
                << " to avoid conflicts between non adjacent subfields";
    ...
    void SpacePartition::stablishBoundaries() {
        ...
        _sections[1] = Rectangle<int>(_ownedArea._size / 2,
                                      Point2D<int>(_ownedArea._origin._x + _ownedArea._size._width / 2,
                                                   _ownedArea._origin._y));
        _sections[2] = Rectangle<int>(_ownedArea._size / 2, Point2D<int>(_ownedArea._origin._x, _ownedArea._origin._y +
                                                                                                _ownedArea._size._height /
                                                                                                2));
        _sections[3] = Rectangle<int>(_ownedArea._size / 2,
                                      Point2D<int>(_ownedArea._origin._x + _ownedArea._size._width / 2,
    ...
    void SpacePartition::stepSection(const int &sectionIndex) {
        ...
        log_DEBUG(logName.str(),
                  getWallTime() << " beginning step: " << _world->getCurrentStep() << " section: " << sectionIndex);
...
#pragma omp parallel for
        ...
        for (size_t i = 0; i < agentsToExecute.size(); i++) {
            ...
            log_DEBUG(logName.str(),
                      getWallTime() << " agent: " << agent << " being executed at index: " << sectionIndex
                                    << " of task: " << _id << " in step: " << _world->getCurrentStep());

            agentsToExecute.at(i)->executeActions();
            agentsToExecute.at(i)->updateState();
            log_DEBUG(logName.str(),
                      getWallTime() << " agent: " << agent << " has been executed at index: " << sectionIndex
                                    << " of task: " << _id << " in step: " << _world->getCurrentStep());

            if (!_ownedArea.contains(agent->getPosition()) && !willBeRemoved(agent->getId())) {
                log_DEBUG(logName.str(),
                          getWallTime() << " migrating agent: " << agent << " being executed at index: " << sectionIndex
                                        << " of task: " << _id);
                ...
                std::cout << _id << "  ag_T_Send: " << agentsToSend.size() << "  n_ag: " << _world->getNumberOfAgents()
                          << "  Ov_ag: " << _overlapAgents.size() << "\n";
        ...
        log_DEBUG(logName.str(), getWallTime() << " sending agents in section: " << sectionIndex << " and step: "
                                               << _world->getCurrentStep());
        sendAgents(agentsToSend);
        log_DEBUG(logName.str(), getWallTime() << " has finished section: " << sectionIndex << " and step: "
                                               << _world->getCurrentStep());

        log_DEBUG(logName.str(),
                  getWallTime() << " executed step: " << _world->getCurrentStep() << " section: " << sectionIndex
                                << " in zone: " << _sections[sectionIndex] << " with num executed agents: "
                                << numExecutedAgents << " total agents: "
                                << std::distance(_world->beginAgents(), _world->endAgents()) << " and overlap agents: "
                                << _overlapAgents.size());
    }
    ...
    void SpacePartition::sendAgents(AgentsList &agentsToSend) {
        ...
        log_DEBUG(logName.str(),
                  getWallTime() << " step: " << _world->getCurrentStep() << " sendAgent: " << agentsToSend.size()
                                << " agents");

        // for each neighbor, we send the number of agents to send
        for (MpiFactory::TypesMap::iterator itType = MpiFactory::instance()->beginTypes();
             itType != MpiFactory::instance()->endTypes(); itType++) {
            log_DEBUG(logName.str(),
                      getWallTime() << " step: " << _world->getCurrentStep() << " sendAgent - checking mpi type: "
                                    << itType->first);
            ...
            for (size_t i = 0; i < _neighbors.size(); i++) {
                ...
                log_DEBUG(logName.str(),
                          getWallTime() << " step: " << _world->getCurrentStep() << " sendAgent - sending num agents: "
                                        << numAgents << " to: " << _neighbors[i]);
                ...
                while (it != agentsToNeighbors[i].end()) {
                    ...
                    log_DEBUG(logName.str(),
                              getWallTime() << " step: " << _world->getCurrentStep() << " sendAgent - sending agent: "
                                            << *it << " to: " << _neighbors[i]);
        ...
        log_DEBUG(logName.str(), getWallTime() << " step: " << _world->getCurrentStep()
                                               << " sendAgent -  end checking agents to send: " << agentsToSend.size());
    }
    ...
    void SpacePartition::sendOverlapZones(const int &sectionIndex, const bool &entireOverlap) {
        ...
        log_DEBUG(logName.str(),
                  getWallTime() << " step: " << _world->getCurrentStep() << "/" << sectionIndex << " sendOverlapZones");
        ...
        for (size_t d = 0; d < _world->getNumberOfRasters(); d++) {
            ...
            log_DEBUG(logName.str(), getWallTime() << " step: " << _world->getCurrentStep() << "/" << sectionIndex
                                                   << " sending raster: " << d);
            for (size_t i = 0; i < neighborsToUpdate.size(); i++) {
                ...
                if (entireOverlap) {
                    ...
                    log_DEBUG(logName.str(),
                              getWallTime() << " step: " << _world->getCurrentStep() << "/" << sectionIndex
                                            << " send entire overlap: " << send->_overlap << " to: "
                                            << neighborsToUpdate[i]);
                } else {
                    send->_overlap = getInternalOverlap(neighborsToUpdate[i]);
                    log_DEBUG(logName.str(),
                              getWallTime() << " step: " << _world->getCurrentStep() << "/" << sectionIndex
                                            << " send partial overlap: " << send->_overlap << " to: "
                                            << neighborsToUpdate[i]);
                }
                ...
                for (size_t n = 0; n < send->_data.size(); n++) {
                    ...
                    log_EDEBUG(logName.str(),
                               "\t" << getWallTime() << " step: " << _world->getCurrentStep() << "/" << sectionIndex
                                    << " send index: " << index << " in global pos: " << index + _boundaries._origin
                                    << " value: " << send->_data.at(n));
                }
                ...
                log_DEBUG(logName.str(), getWallTime() << " step: " << _world->getCurrentStep() << " raster: " << d
                                                       << " data sent to: " << _neighbors[i]);
            }
            log_DEBUG(logName.str(),
                      getWallTime() << " step: " << _world->getCurrentStep() << "/" << sectionIndex << " raster: " << d
                                    << " sent");
        }
        log_DEBUG(logName.str(), getWallTime() << " step: " << "/" << sectionIndex << _world->getCurrentStep()
                                               << " sendOverlapZones ended");
    }
    ...
    void SpacePartition::sendMaxOverlapZones() {
        ...
        for (size_t d = 0; d < _world->getNumberOfRasters(); d++) {
            ...
            log_DEBUG(logName.str(),
                      getWallTime() << " step: " << _world->getCurrentStep() << " sending max raster: " << d);
            for (size_t i = 0; i < _neighbors.size(); i++) {
                MpiOverlap *send = new MpiOverlap;
                send->_overlap = getInternalOverlap(_neighbors[i]);
                send->_data.resize(send->_overlap._size._width * send->_overlap._size._height);
                log_DEBUG(logName.str(),
                          getWallTime() << " step: " << _world->getCurrentStep() << " will send max overlap of: " << d
                                        << " to: " << _neighbors[i] << " with size: " << send->_data.size()
                                        << " and zone: " << send->_overlap << " to " << _neighbors[i]);
                const Rectangle<int> &overlapZone = send->_overlap;
                for (size_t n = 0; n < send->_data.size(); n++) {
                    Point2D<int> index(overlapZone._origin._x + n % overlapZone._size._width,
                                       overlapZone._origin._y + n / overlapZone._size._width);
                    ...
                    log_EDEBUG(logName.str(),
                               "\t" << getWallTime() << " step: " << _world->getCurrentStep() << " send index: "
                                    << index << " in global pos: " << index + _boundaries._origin << " max value: "
                                    << send->_data.at(n));
                }
                MPI_Isend(&send->_data[0], send->_data.size(), MPI_INTEGER, _neighbors[i], eRasterMaxData,
                          MPI_COMM_WORLD, &send->_request);
                _sendRequests.push_back(send);
                log_DEBUG(logName.str(), getWallTime() << " step: " << _world->getCurrentStep() << " raster: " << d
                                                       << " max data sent to: " << _neighbors[i]);
            }
            log_DEBUG(logName.str(),
                      getWallTime() << " step: " << _world->getCurrentStep() << " raster: " << d << " max data sent");
        }
        log_DEBUG(logName.str(),
                  getWallTime() << " step: " << _world->getCurrentStep() << " sendMaxOverlapZones ended");
    }
    ...
    void SpacePartition::sendGhostAgents(const int &sectionIndex) {
        ...
        logName << "MPI_agents_world_" << _id;
        log_DEBUG(logName.str(),
                  getWallTime() << " step: " << _world->getCurrentStep() << " send ghost agents for section index: "
                                << sectionIndex);
        ...
        for (size_t i = 0; i < _neighbors.size(); i++) {
            if (needsToBeUpdated(_neighbors[i], sectionIndex)) {
                neighborsToUpdate.push_back(_neighbors[i]);
                log_DEBUG(logName.str(),
                          getWallTime() << " step: " << _world->getCurrentStep() << " section index: " << sectionIndex
                                        << " will send overlap to: " << _neighbors[i]);
            }
        }
        ...
        for (MpiFactory::TypesMap::iterator itType = MpiFactory::instance()->beginTypes();
            ...
            for (size_t i = 0; i < neighborsToUpdate.size(); i++) {
                ...
                for (AgentsList::iterator it = _world->beginAgents(); it != _world->endAgents(); ++it) {
                    ...
                    // TODO refactor!!!
                    log_DEBUG(logName.str(),
                              getWallTime() << " step: " << _world->getCurrentStep() << " agent: " << agent
                                            << " of type: " << itType->first << " test will be removed: "
                                            << willBeRemoved(agent->getId()) << " checking overlap zone: "
                                            << overlapZone << " overlap boundaries: " << _boundaries
                                            << " - test is inside zone: "
                                            << overlapZone.contains(agent->getPosition() - _boundaries._origin));
                    if (agent->isType(itType->first)) {
                        if ((!willBeRemoved(agent->getId())) &&
                            (overlapZone.contains(agent->getPosition() - _boundaries._origin))) {
                            agentsToNeighbors[i].push_back(agent);
                            log_DEBUG(logName.str(),
                                      getWallTime() << " step: " << _world->getCurrentStep() << " sending ghost agent: "
                                                    << agent << " to: " << neighborsToUpdate[i] << " in section index: "
                                                    << sectionIndex);
                        }
                    }
                }
                for (AgentsList::iterator it = _overlapAgents.begin(); it != _overlapAgents.end(); ++it) {
                    ...
                    if (agent->isType(itType->first)) {
                        if ((!willBeRemoved(agent->getId())) &&
                            ...
                            agentsToNeighbors[i].push_back(agent);
                            log_DEBUG(logName.str(), getWallTime() << " step: " << _world->getCurrentStep()
                                                                   << " will send modified ghost agent: " << agent
                                                                   << " to: " << neighborsToUpdate[i]
                                                                   << " in section index: " << sectionIndex
                                                                   << " and step: " << _world->getCurrentStep());
                        }
                    }
                }
                ...
                int numAgents = agentsToNeighbors[i].size();
                log_DEBUG(logName.str(),
                          getWallTime() << " step: " << _world->getCurrentStep() << " sending num ghost agents: "
                                        << numAgents << " to : " << neighborsToUpdate[i] << " in step: "
                                        << _world->getCurrentStep() << " and section index: " << sectionIndex);
                ...
                for (AgentsList::iterator it = agentsToNeighbors[i].begin(); it != agentsToNeighbors[i].end(); ++it) {
                    ...
                    void *package = agent->fillPackage();
                    log_DEBUG(logName.str(),
                              getWallTime() << " step: " << _world->getCurrentStep() << " sending ghost agent: " << *it
                                            << " from: " << _id << " to: " << neighborsToUpdate[i]);
    ...
    void SpacePartition::receiveGhostAgents(const int &sectionIndex) {
        ...
        for (MpiFactory::TypesMap::iterator itType = MpiFactory::instance()->beginTypes();
            ...
            for (size_t i = 0; i < neighborsToUpdate.size(); i++) {
                log_DEBUG(logName.str(), getWallTime() << " AA neigh to update: " << neighborsToUpdate[i] << " type: "
                                                       << itType->first);
                AgentsList newGhostAgents;
                int numAgentsToReceive;
                MPI_Status status;

                int error = MPI_Recv(&numAgentsToReceive, 1, MPI_INTEGER, neighborsToUpdate[i], eNumGhostAgents,
                ...
                log_DEBUG(logName.str(),
                          getWallTime() << " step: " << _world->getCurrentStep() << " has received message from "
                                        << neighborsToUpdate[i] << ", num ghost agents: " << numAgentsToReceive);
                for (int j = 0; j < numAgentsToReceive; j++) {
                    void *package = MpiFactory::instance()->createDefaultPackage(itType->first);
                    error = MPI_Recv(package, 1, *agentType, neighborsToUpdate[i], eGhostAgent, MPI_COMM_WORLD,
                                     &status);
                    if (error != MPI_SUCCESS) {
                        std::stringstream oss;
                        oss << "SpacePartition::receiveGhostAgents - " << _id << " error in MPI_Recv: " << error;
                    ...
                    log_DEBUG(logName.str(),
                              getWallTime() << " step: " << _world->getCurrentStep() << " has received ghost agent: "
                                            << agent << " number: " << j << " from: " << neighborsToUpdate[i]
                                            << " in section index: " << sectionIndex << " and step: "
                                            << _world->getCurrentStep());
                    delete package;
                    ...
                    if (it != _world->endAgents()) {
                        log_DEBUG(logName.str(), getWallTime() << " step: " << _world->getCurrentStep()
                                                               << " has received update of own agent: " << *it
                                                               << " in step: " << _world->getCurrentStep());
                ...
                log_DEBUG(logName.str(), getWallTime() << " step: " << _world->getCurrentStep()
                                                       << " num ghost agents sent for neighbor: "
                                                       << neighborsToUpdate[i] << " in section index: " << sectionIndex
                                                       << ": " << newGhostAgents.size());
                ...
                for (it = newGhostAgents.begin(); it != newGhostAgents.end(); ++it) {
                    ...
                    log_DEBUG(logName.str(),
                              getWallTime() << " step: " << _world->getCurrentStep() << " in section index: "
                                            << sectionIndex << " adding ghost agent: " << agent);
                    _overlapAgents.push_back(agent);
                }
            }
        }
    ...
    void SpacePartition::receiveAgents(const int &sectionIndex) {
        ...
        logName << "MPI_agents_world_" << _id;
        log_DEBUG(logName.str(),
                  getWallTime() << " step: " << _world->getCurrentStep() << " receiving agents for section index: "
                                << sectionIndex);
        for (MpiFactory::TypesMap::iterator itType = MpiFactory::instance()->beginTypes();
             itType != MpiFactory::instance()->endTypes(); itType++) {
            log_DEBUG(logName.str(),
                      getWallTime() << " step: " << _world->getCurrentStep() << " receiveAgent - checking mpi type: "
                                    << itType->first);
            MPI_Datatype *agentType = itType->second;

            for (size_t i = 0; i < _neighbors.size(); i++) {
                log_DEBUG(logName.str(), getWallTime() << " step: " << _world->getCurrentStep()
                                                       << " receiveAgent - checking mpi type: " << itType->first
                                                       << " for neighbor: " << _neighbors[i]);

                int numAgentsToReceive;
                MPI_Status status;
                int error = MPI_Recv(&numAgentsToReceive, 1, MPI_INTEGER, _neighbors[i], eNumAgents, MPI_COMM_WORLD,
                                     &status);
                if (error != MPI_SUCCESS) {
                    std::stringstream oss;
                ...
                log_DEBUG(logName.str(), getWallTime() << " receiveAgents - received message from " << _neighbors[i]
                                                       << ", num agents: " << numAgentsToReceive);
                for (int j = 0; j < numAgentsToReceive; j++) {
                    void *package = MpiFactory::instance()->createDefaultPackage(itType->first);
                    ...
                    log_DEBUG(logName.str(), getWallTime() << " step: " << _world->getCurrentStep()
                                                           << " receiveAgents - received agent: " << agent
                                                           << " number: " << j << " from: " << _neighbors[i]);
                }
            }
        }
    }
        ...
        for (size_t d = 0; d < _world->getNumberOfRasters(); d++) {
            ...
            for (size_t i = 0; i < neighborsToUpdate.size(); i++) {
                ...
                if (entireOverlap) {
                    ...
                    log_DEBUG(logName.str(),
                              getWallTime() << " step: " << _world->getCurrentStep() << "/" << sectionIndex
                                            << " receive entire overlap: " << receive->_overlap << " from "
                                            << neighborsToUpdate[i]);
                } else {
                    receive->_overlap = getExternalOverlap(neighborsToUpdate[i]);
                    log_DEBUG(logName.str(),
                              getWallTime() << " step: " << _world->getCurrentStep() << "/" << sectionIndex
                                            << " receive external overlap: " << receive->_overlap << " from "
                                            << neighborsToUpdate[i]);
                }
                receive->_data.resize(receive->_overlap._size._width * receive->_overlap._size._height);
                log_DEBUG(logName.str(), getWallTime() << " step: " << _world->getCurrentStep() << "/" << sectionIndex
                                                       << " will receive overlap from: " << neighborsToUpdate[i]
                                                       << " with size: " << receive->_data.size() << " and zone: "
                                                       << receive->_overlap);
                MPI_Irecv(&receive->_data[0], receive->_data.size(), MPI_INTEGER, neighborsToUpdate[i], eRasterData,
                          MPI_COMM_WORLD, &receive->_request);
    ...
    void SpacePartition::receiveMaxOverlapData() {
        ...
        for (size_t d = 0; d < _world->getNumberOfRasters(); d++) {
            ...
            log_DEBUG(logName.str(),
                      getWallTime() << " step: " << _world->getCurrentStep() << " receiving max raster: " << d);
            for (size_t i = 0; i < _neighbors.size(); i++) {
                MpiOverlap *receive = new MpiOverlap;
                receive->_rasterName = _world->getRasterName(d);
                ...
                log_DEBUG(logName.str(),
                          getWallTime() << " step: " << _world->getCurrentStep() << " will receive max overlap to: "
                                        << _neighbors[i] << " with size: " << receive->_data.size() << " and zone: "
                                        << receive->_overlap << " from " << _neighbors[i]);
                MPI_Irecv(&receive->_data[0], receive->_data.size(), MPI_INTEGER, _neighbors[i], eRasterMaxData,
                          MPI_COMM_WORLD, &receive->_request);
                _receiveRequests.push_back(receive);
            }
            log_DEBUG(logName.str(), getWallTime() << " step: " << _world->getCurrentStep() << " raster: " << d
        ...
        log_DEBUG(logName.str(),
                  getWallTime() << " step: " << _world->getCurrentStep() << " receiveMaxOverlapData ended");
    }
        ...
        while (_receiveRequests.size() != 0) {
            ...
            while (it != _receiveRequests.end()) {
                ...
                if (finished) {
                    ...
                    log_DEBUG(logName.str(),
                              getWallTime() << " receive request finished, lacking: " << _receiveRequests.size()
                                            << " requests");

                    const Rectangle<int> &overlapZone = receive->_overlap;
                    log_DEBUG(logName.str(),
                              getWallTime() << " processing request for overlap: " << overlapZone << " raster: "
                                            << receive->_rasterName << " for max values");
                    for (size_t i = 0; i < receive->_data.size(); i++) {
                        Point2D<int> index(overlapZone._origin._x + i % overlapZone._size._width,
                                           overlapZone._origin._y + i / overlapZone._size._width);
                        if (updateMaxValues) {
                            log_EDEBUG(logName.str(), "\t" << getWallTime() << " step: " << _world->getCurrentStep()
                                                           << " receive index: " << index << " max value: "
                                                           << receive->_data.at(i));
                            _world->getDynamicRaster(receive->_rasterName).setMaxValue(index, receive->_data.at(i));
                        } else {
                            log_EDEBUG(logName.str(), "\t" << getWallTime() << " step: " << _world->getCurrentStep()
                                                           << " receive index: " << index << " current value: "
                                                           << receive->_data.at(i));
                    ...
                    if (updateMaxValues) {
                        log_DEBUG(logName.str(), getWallTime() << " receive request finished for max values of raster: "
                                                               << receive->_rasterName);
                    } else {
                        log_DEBUG(logName.str(),
                                  getWallTime() << " receive request finished for current values of raster: "
                                                << receive->_rasterName);
                    }
                    delete receive;
                }
    ...
    void SpacePartition::executeAgents() {
        ...
        log_DEBUG(logName.str(),
                  getWallTime() << " step: " << _world->getCurrentStep() << " has executed update overlap");
        ...
        for (int sectionIndex = 0; sectionIndex < 4; sectionIndex++) {
            ...
            log_DEBUG(logNameMpi.str(),
                      getWallTime() << " executing step: " << _world->getCurrentStep() << " and section: "
                                    << sectionIndex << " has been executed");

            receiveAgents(sectionIndex);
            log_DEBUG(logNameMpi.str(),
                      getWallTime() << " executing step: " << _world->getCurrentStep() << " and section: "
                                    << sectionIndex << " has received agents");

            sendGhostAgents(sectionIndex);
            log_DEBUG(logNameMpi.str(),
                      getWallTime() << " executing step: " << _world->getCurrentStep() << " and section: "
                                    << sectionIndex << " sent ghosts");

            receiveGhostAgents(sectionIndex);
            log_DEBUG(logNameMpi.str(),
                      getWallTime() << " executing step: " << _world->getCurrentStep() << " and section: "
                                    << sectionIndex << " received ghosts");
            ...
            log_DEBUG(logNameMpi.str(),
                      getWallTime() << " executing step: " << _world->getCurrentStep() << " and section: "
                                    << sectionIndex << " sent overlap");

            receiveOverlapData(sectionIndex);
            log_DEBUG(logNameMpi.str(),
                      getWallTime() << " executing step: " << _world->getCurrentStep() << " and section: "
                                    << sectionIndex << " received overlap");
    ...
    Rectangle<int> SpacePartition::getInternalOverlap(const int &id) const {
        ...
        logName << "MPI_raster_world_" << _id;
        log_DEBUG(logName.str(),
                  getWallTime() << " internal overlap with: " << id << " and diff: " << diff << " - " << result);
        return result;
    }
    ...
    Rectangle<int> SpacePartition::getExternalOverlap(const int &id) const {
        ...
        logName << "MPI_raster_world_" << _id;
        log_DEBUG(logName.str(),
                  getWallTime() << " external overlap with: " << id << " and diff: " << diff << " - " << result);
        return result;
    }
    ...
    Rectangle<int> SpacePartition::getOverlap(const int &id, const int &sectionIndex) const {
        ...
        logName << "MPI_raster_world_" << _id;
        log_DEBUG(logName.str(),
                  getWallTime() << " overlap with: " << id << " and diff: " << diff << " in section Index: "
                                << sectionIndex << " - " << result);
        return result;
    }
    ...
    int SpacePartition::countNeighbours(Agent *target, const double &radius, const std::string &type) {
        int numAgents = for_each(_world->beginAgents(), _world->endAgents(),
                                 aggregatorCount < std::shared_ptr < Agent > > (radius, *target, type))._count;
        int numOverlapAgents = for_each(_overlapAgents.begin(), _overlapAgents.end(),
                                        aggregatorCount < std::shared_ptr < Agent > > (radius, *target, type))._count;
    ...
    AgentsVector SpacePartition::getNeighbours(Agent *target, const double &radius, const std::string &type) {
        AgentsVector agentsVector = for_each(_world->beginAgents(), _world->endAgents(),
                                             aggregatorGet < std::shared_ptr < Agent > >
                                             (radius, *target, type))._neighbors;
        AgentsVector overlapAgentsVector = for_each(_overlapAgents.begin(), _overlapAgents.end(),
                                                    aggregatorGet < std::shared_ptr < Agent > >
                                                    (radius, *target, type))._neighbors;
